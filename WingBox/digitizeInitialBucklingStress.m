...
x_data_max = 1.2;  % Data x-coordinate of the maximum x-value
y_data_min = 0;   % Data y-coordinate of the origin
y_data_max = 1.8;   % Data y-coordinate of the maximum y-value
...
% digitized_data = struct(...
%     'ts_t_ratio', {}, ... % Store the ts/t ratio
%     'x_data', {}, ...     % Store x-values (As/bt)
%     'y_data', {});        % Store y-values (σ_cr/σ₀)
ts_t = input("input ts_t ratio: ");
n = length(digitized_data);
digitized_data(n+1).ts_t_ratio = ts_t;
digitized_data(n+1).x_data = x_data;
digitized_data(n+1).y_data = y_data;
figure(1)
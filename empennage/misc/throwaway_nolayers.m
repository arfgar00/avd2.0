...
for i = 1:length(n_values)
    ...
    % properties per ply
    t = 2e-4; % ply thickness
    sigma_c = 1780e6; % strength in compression
    tau_xy = 5.61e9; % strength in shear
    a_b = a / b; % panel aspect ratio
    

    qFS = -(shear_vals ./ (2 .* h)) + ( transpose(flip(combined_moment_temp)) ./ (2 .* c .* h) )
    qRS = -(shear_vals ./ (2 .* h)) - ( transpose(flip(combined_moment_temp)) ./ (2 .* c .* h) )
    ...
    for j = 1:size(shear_vals, 1) % Loop through rows of shear_vals
        ...
        covers_and_webs_table.No_Plys = uint16(covers_and_webs_table.No_Plys); % Convert to integer type
